...
export const fetchUsersV2Data = async (chainId, provider, usersAddress, contractAddresses) => {
  ...
  const values = await batch(
    provider,
    etherplexV2DaiPoolContract.committedBalanceOf(usersAddress).openBalanceOf(usersAddress),
    etherplexV2UsdcPoolContract.committedBalanceOf(usersAddress).openBalanceOf(usersAddress),
