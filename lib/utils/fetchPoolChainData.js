...
import { CUSTOM_CONTRACT_ADDRESSES, DEFAULT_TOKEN_PRECISION } from 'lib/constants'
...
export const fetchPoolChainData = async ({ provider, poolGraphData, chainId }) => {
  try {
    ...
    if (sablierStreamId && CUSTOM_CONTRACT_ADDRESSES[chainId].Sablier) {
...
const _fetchReserveRate = async (provider, reserveRegistryAddress) => {
  try {
    ...
    const { reserveData } = await batch(provider, reserveContract.reserveRateMantissa())
