...
export const fetchPoolChainData = async ({ provider, poolGraphData, chainId }) => {
  try {
    ...
    if (tempReserveAddress !== ethers.constants.AddressZero) {
      reserveRate = await _fetchReserveRate(provider, tempReserveAddress, poolAddress)
    }
...
const _fetchPoolData = async (prizePoolContract, prizeStrategyContract, provider) => {
  ...
  // # Note: This needs to be run in a separate multichain call since the non-constant
  // captureAwardBalance is modifying the result of reserveTotalSupply
  const poolReserveData = await batch(provider, prizePoolContract.reserveTotalSupply())

  return {
    ...poolData,
    awardBalance: poolData.prizePool.captureAwardBalance[0],
    reserveTotalSupply: poolReserveData.prizePool.reserveTotalSupply[0],
    isRngRequested: poolData.prizeStrategy.isRngRequested[0],
...
const _fetchSablierStreamData = async (provider, chainId, sablierStreamId, prizeStrategyData) => {
  try {
    ...
    return {
      ...
      tokenAddress: sablierValues.sablier.getStream.tokenAddress.toLowerCase(),
      startTime: sablierValues.sablier.getStream.startTime,
      stopTime: sablierValues.sablier.getStream.stopTime,
      ratePerSecond: sablierValues.sablier.getStream.ratePerSecond,
...
const _fetchReserveRate = async (provider, reserveRegistryAddress, poolAddress) => {
  try {
    ...
    const { reserveData } = await batch(provider, reserveContract.reserveRateMantissa(poolAddress))
    return reserveData.reserveRateMantissa[0]
  } catch (e) {
    console.warn(e)
