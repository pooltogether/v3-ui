...
export const fetchExternalErc1155Awards = async ({ provider, erc1155Awards, balanceOfAddress }) => {
  ...
  for (i = 0; i < erc1155Awards?.length; i++) {
    ...
    const tokenId = ethers.BigNumber.from(award.tokenIds[0])
    etherplexTokenContract = contract(address, ERC1155Abi, address)

    batchCalls.push(etherplexTokenContract.balanceOf(balanceOfAddress, tokenId))
  }
