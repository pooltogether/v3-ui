  ...
  if (ticketValues) {
    chainIds.forEach((chainId) => {
      ...
      pools.forEach((pool) => {
        ...
        const formatTicketObj = (chainId, userTicketData) => ({
          chainId,
          amountUnformatted: ethers.BigNumber.from(userTicketData.balanceOf[0]),
          amount: formatUnits(userTicketData.balanceOf[0], userTicketData.decimals[0]),
          totalSupplyUnformatted: ethers.BigNumber.from(userTicketData.totalSupply[0]),
          totalSupply: formatUnits(userTicketData.totalSupply[0], userTicketData.decimals[0]),
          decimals: userTicketData.decimals[0],
          address: ticketAddress
          // name: userTicketData.name[0]
        })

        formattedTicketData[chainId].push(formatTicketObj(chainId, userTicketData))
        formattedTicketData[chainId].push(formatTicketObj(chainId, userSponsorshipData))
      })
