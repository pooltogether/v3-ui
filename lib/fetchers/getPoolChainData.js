...
const formatPoolChainData = (
  ...
  poolGraphData.forEach((pool) => {
    ...
    const formattedPoolChainData = {
      ...
      prizePool: {
        awardBalance: formatUnits(
          firstBatchValues[prizePoolAddress].captureAwardBalance[0],
          pool.tokens.underlyingToken.decimals
        ),
        awardBalanceUnformatted: firstBatchValues[prizePoolAddress].captureAwardBalance[0],
        reserveTotalSupply: secondBatchValues[prizePoolAddress].reserveTotalSupply[0]
      },
      prize: {
    ...
    if (pool.tokenListener.address) {
      ...
      formattedPoolChainData.tokenListener = {
        dripRatePerSecond: tokenListenerData.dripRatePerSecond[0],
        measure: tokenListenerData.measure[0],
        token: {
          address: tokenFaucetDripAssetAddress,
          balance: formatUnits(tokenListenerData2.balanceOf[0], tokenListenerData2.decimals[0]),
          balanceUnformatted: tokenListenerData2.balanceOf[0],
    ...
    // External ERC20 awards
    const externalErc20Awards = {}
    if (pool.prize.externalErc20Awards.length > 0) {
      pool.prize.externalErc20Awards.forEach((erc20) => {
        const erc20AwardData =
          firstBatchValues[getExternalErc20AwardBatchName(prizePoolAddress, erc20.address)]
        externalErc20Awards[prizePoolAddress] = {
          ...externalErc20Awards[prizePoolAddress],
          [erc20.address]: {
            balance: erc20AwardData.balanceOf[0],
            balanceUnformatted: erc20AwardData.balanceOf[0]
          }
        }
      })
    }
    formattedPoolChainData.externalErc20Awards = externalErc20Awards

    // External ERC721 awards
    const externalErc721Awards = {}
    if (pool.prize.externalErc721Awards.length > 0) {
      pool.prize.externalErc721Awards.forEach((erc721) => {
        erc721.tokenIds.forEach((tokenId) => {
          ...
          externalErc721Awards[erc721.address] = {
            ...externalErc721Awards[erc721.address],
            uri: erc721Uri,
            [tokenId]: {
              balance: erc721AwardData.balanceOf[0]
            }
          }
        })
      })
    }
    formattedPoolChainData.externalErc721Awards = externalErc721Awards
    ...
    if (pool.prize.sablierStream.id) {
      ...
      formattedPoolChainData.sablier = {
    ...
    if (lootBoxAddress && pool.prize.externalErc721Awards.length > 0) {
      ...
      if (lootBox) {
        formattedPoolChainData.lootbox = []
        lootBox.tokenIds.forEach((tokenId) => {
          formattedPoolChainData.lootbox.push({
            address:
              firstBatchValues[getLootBoxBatchName(lootBoxAddress, tokenId)].computeAddress[0],
            id: tokenId
          })
        })
      }
