...
const calculateTotalPrizeValuePerPool = (poolData) => {
  poolData.forEach((pool) => {
    ...
    calculateExternalErc20AwardTotalValuesUsd(pool)

    // Calculate lootBox award value
    calculateLootBoxTotalValuesUsd(pool)

    // Calculate yield prize
    calculateYieldTotalValuesUsd(pool)

    // Calculate sablier prize
    calculateSablierTotalValueUsd(pool)
...
const calculateExternalErc20AwardTotalValuesUsd = (pool) => {
...
const calculateLootBoxTotalValuesUsd = (pool) => {
  const lootBoxTotalValueUsdScaled =
    pool.prize.lootBox?.contents?.reduce((total, box) => {
      if (box.erc20Tokens.length > 0) {
        return total.add(box.erc20Tokens.reduce(addScaledTokenValueToTotal, ethers.constants.Zero))
      }
      return total
...
const calculateYieldTotalValuesUsd = (pool) => {
...
const calculateSablierTotalValueUsd = (pool) => {
