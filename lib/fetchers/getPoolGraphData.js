...
export const getPoolGraphData = async (chainId, poolContracts, blockNumber = -1) => {
  ...
  const data = await Promise.all(
    subgraphVersions.map((version) => {
      ...
      return client.request(query, { poolAddresses })
    })
  )

  return data.flatMap(({ prizePools }) =>
  ...
  const formattedData = {
    ...
    tokens: {
      ticket: {
        ...
        totalSupplyUnformatted: ticket.totalSupply,
      ...
      sponsorship: {
        ...
        totalSupplyUnformatted: sponsorship.totalSupply,
