...
export const getPoolPrizesData = async (chainId, provider, poolContract, page, pageSize) => {
  const variables = {
    poolAddress: poolContract.prizePool.address.toLowerCase(),
    first: pageSize,
    skip: (page - 1) * pageSize
  }

  const graphQLClient = getSubgraphClientFromChainIdAndVersion(
    chainId,
...
export const getPoolPrizeData = async (chainId, provider, poolContract, prizeNumber) => {
  const variables = {
    poolAddress: poolContract.prizePool.address.toLowerCase(),
    prizeId: `${poolContract.prizePool.address.toLowerCase()}-${prizeNumber}`
  }
