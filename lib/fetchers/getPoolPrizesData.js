...
export const getPoolPrizesData = async (chainId, poolContract, page, pageSize) => {
  ...
  try {
    ...
    // TODO: Bug in the graph returning null for token ids for loot boxes so we can't get the contents easily
    // Need to also add the erc20's in there into the token price data fetching. getErc20AddressesByBlockNumberFromPrizes
    ...
    // const graphLootBox = lootBoxesData.lootBoxes[0]
...
/**
 * Pulls the loot box token id from the awardedExternalErc721Nfts from a prize
 * @param {*} chainId
 * @param {*} prize
 * @returns
 */
