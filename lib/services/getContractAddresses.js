...
export const getContractAddresses = (chainId) => {
  ...
  try {
    batPoolAddress = contractAddresses[chainId].bat.prizePool
    ...
    if (!batPoolAddress) {
      throw new Error(`Unable to find BAT prize pool contract for chainId: ${chainId}`)
    }
  } catch (e) {
    throw e
  }
  ...
  return {
    pools: [
      daiPoolAddress.toLowerCase(),
      batPoolAddress.toLowerCase()
    ],
    'PT-cDAI': daiPoolAddress.toLowerCase(),
    'PT-cBAT': batPoolAddress.toLowerCase(),
