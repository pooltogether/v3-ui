...
export const compileLootBoxErc20s = (erc20Awards, uniswapPriceData) => {
  ...
  erc20s.forEach(key => {
    ...
    let balanceBN = ethers.utils.bigNumberify(0)
    let balanceFormatted = ''
    if (balance) {
      balanceBN = ethers.utils.bigNumberify(balance)
      balanceFormatted = ethers.utils.formatUnits(balanceBN, parseInt(tokenDecimals, 10))
    }
