...
export const getUsersTicketBalance = (pool, playerData) => {
  ...
  let usersTicketBalance = ''
  let usersTicketBalanceBN = ethers.BigNumber.from(0)

  if (ticketAddress && userAddress && playerData) {
    const playerTicketData = playerData?.controlledTokenBalances.find(
      (data) => data.controlledToken.id === ticketAddress
    )
    ...
    if (playerTicketData && underlyingCollateralDecimals) {
      ...
      usersTicketBalanceBN = ethers.BigNumber.from(playerTicketData.balance)
    }
  }
