...
// import { lastRevealedDrawIdResolver } from 'lib/apollo/query/lastRevealedDrawIdResolver'
...
export async function apolloClient() {
  ...
  const tightbeam = new Tightbeam({
    providerSource: readProvider,
    
    txProviderSource: writeProvider,
    abiMapping,
    defaultFromBlock
  })
  ...
  const initCache = () => {
    cache.writeData(tightbeam.defaultCacheData())
  }
  initCache()
