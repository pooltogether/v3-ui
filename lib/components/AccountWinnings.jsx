...
export const AccountWinnings = () => {
  ...
  awardedControlledTokens = awardedControlledTokens.filter((awardedControlledToken) =>
    contractAddresses.pools.includes(awardedControlledToken.prize.prizePool.id)
  )
    ...
    <>
      ...
      <div className='xs:mt-3 bg-accent-grey-4 rounded-lg xs:mx-0 px-2 sm:px-6 py-3'>
        ...
        <div className='text-inverse flex flex-col justify-between xs:px-2'>
          <table className='table-fixed w-full text-xxs xs:text-base sm:text-xl mt-6'>
            <tbody></tbody>
          </table>
        </div>

        <div className='text-xxxs xs:text-xxs sm:text-xs opacity-70 px-2 xs:px-4 py-4'>
          {t('seeAllPastWinnersOn')}{' '}
          <Link href='/prizes/PT-cDAI' shallow>
            <a className='text-accent-grey-1 underline'>{t('pastWinnersPrizeHistoryLink')}</a>
          </Link>
        </div>
      </div>
    </>
