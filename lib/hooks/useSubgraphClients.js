...
export const useSubgraphClients = (versions) => {
  const chainId = useWalletChainId()
  return versions.map((version) => POOLTOGETHER_SUBGRAPHS[chainId][version])
}

export const getSubgraphClientFromVersion = (chainId, version) =>
...
export const getSubgraphClientByVersionFromContract = (poolContract, chainId) =>
  getSubgraphClientsByVersionFromContracts([poolContract], chainId)?.[poolContract.subgraphVersion]
...
// TODO: this method theGraphCustomFetch() should only exist in one place
// Currently it's also in `useTheGraphCustomFetch.js` hook file as well
