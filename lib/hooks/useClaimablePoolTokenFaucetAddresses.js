...
const useFetchPoolTokenFaucets = (poolAddresses) => {
  ...
  return useQuery(
    [QUERY_KEYS.claimablePoolTokenFaucetAddresses, chainId, poolAddresses],
    async () => {
...
// like `usePools_OLD` but that is only subgraph data, `usePool` can't be looped over as it's a hook.
