...
import { APP_ENVIRONMENTS, useIsTestnets, useReadProvider } from '@pooltogether/hooks'
...
export const useUserLPChainData = (stakingPool, stakingPoolChainData, usersAddress) => {
  ...
  const readProvider = useReadProvider(chainId)
  const enabled = Boolean(usersAddress) && !!readProvider && Boolean(stakingPoolChainData)
