...
export const useClaimablePool = (poolSymbol) => {
  ...
  const { tokenListener: tokenFaucetAddress } = pool
  const [claimablePoolRefetchFns, setClaimablePoolRefetchFns] = useAtom(claimablePoolRefetchFnsAtom)

  const { refetch, data, isFetching, isFetched, error } = useFetchClaimablePoolData(
    tokenFaucetAddress
  )
...
function useFetchClaimablePoolData (tokenFaucetAddress) {
  ...
  return useQuery(
    ...
    async () => {
      return getClaimablePoolData(readProvider, usersAddress, tokenFaucetAddress, chainId)
    },
    {
      enabled,
...
async function getClaimablePoolData (provider, usersAddress, tokenFaucetAddress, chainId) {
