...
export const useAllPlayerTickets = (playersAddress) => {
  const { data: accountData, isFetched: accountDataIsFetched } = useMultiversionAccount(
    playersAddress
  )
  ...
  if (accountDataIsFetched && poolsIsFetched) {
    playerTickets = accountData?.controlledTokenBalances?.map(mapPlayerTickets)
  }

  return { data: playerTickets, isFetched: poolsIsFetched && accountDataIsFetched }
}
