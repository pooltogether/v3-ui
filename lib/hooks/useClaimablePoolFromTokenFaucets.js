...
export const useClaimablePoolFromTokenFaucets = (usersAddress) => {
  const tokenFaucetAddresses = useClaimablePoolTokenFaucetAddresses()
  ...
  const enabled = Boolean(
    tokenFaucetAddresses && !pauseQueries && usersAddress && !addressError && readProviderIsLoaded
  )
...
async function getClaimablePoolFromTokenFaucets(provider, usersAddress, tokenFaucetAddresses) {
  ...
  // This rinkeby contract is bad and causes annoying etherplex/ethers errors
  // when we try to query data against it:
  if (provider?.network?.chainId === 4) {
    tokenFaucetAddresses = tokenFaucetAddresses.filter(
      (address) => address.toLowerCase() !== '0xe70c3669371be02cde0809df6d9c514804cad2f0'
    )
  }
  ...
  for (const tokenFaucetAddress of tokenFaucetAddresses) {
    try {
      ...
      const { tokenFaucet } = await batch(
        provider,
        tokenFaucetContract.dripRatePerSecond().asset().measure().claim(usersAddress)
      )
