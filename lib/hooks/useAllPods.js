...
import { usePodChainIds, useReadProviders, useRefetchInterval } from '@pooltogether/hooks'
import {
  addBigNumbers,
  calculateUsersOdds,
...
const getPodBalances = async (chainId, provider, pods) => {
  ...
  pods.map((pod) => {
    // Pod Shares
    const podShareContract = contract(getPodKey(pod.pod.address), PodAbi, pod.pod.address)

    batchCalls.push(podShareContract.totalSupply().getPricePerShare().balance().decimals())
  })
