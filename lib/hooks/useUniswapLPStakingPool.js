...
export const useUniswapStakingPool = () => {
  ...
  return useQuery(
    [QUERY_KEYS.uniswapLPStakingPool, chainId, usersAddress],
    () => getUniswapStakingPoolData(chainId, readProvider, usersAddress),
    { enabled }
  )
}

const getUniswapStakingPoolData = async (chainId, readProvider, usersAddress) => {
  const uniswapLPPoolAddress = CUSTOM_CONTRACT_ADDRESSES[chainId].UniswapLPPool
  ...
  const response = await batch(
    ...
    lpTokenContract
      ...
      .decimals()
      .allowance(usersAddress, uniswapLPPoolAddress),
  ...
  const ownershipPercentage = ticketTotalSupplyUnformatted.isZero()
    ? ethers.constants.Zero
    : usersTicketBalanceUnformatted.mul(100).div(ticketTotalSupplyUnformatted)
  ...
  return {
    ...
    user: {
      lpToken: {
        ...
        allowance: response.lpToken.allowance[0]
      },
      tickets: {
