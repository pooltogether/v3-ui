...
export const useAllPoolsKeyedByChainId = () => {
  ...
  return useQuery(getUsePoolsQueryKey(chainIds), async () => await getPoolsByChainIds(chainIds), {
    enabled,
    onSuccess: (data) => populatePerChainIdCache(queryClient, data),
 ...
 * @returns
 */
export const usePoolsByChainId = (chainId) => {
  const { pauseQueries } = useContext(AuthControllerContext)
  const queryClient = useQueryClient()

  const enabled = !pauseQueries
  return useQuery(getUsePoolsQueryKey(chainId), async () => await getPoolsByChainId(chainId), {
    enabled,
    onSuccess: (data) => populatePerPoolCache(chainId, queryClient, data),
    refetchInterval: MAINNET_POLLING_INTERVAL
  })
}
 ...
 * @returns
 */
export const usePoolByChainId = (poolAddress, chainId) => {
  const { pauseQueries } = useContext(AuthControllerContext)
 ...
 * @returns
 */
export const useCurrentPool = (chainId) => {
