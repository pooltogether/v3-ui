...
import { useGovernancePoolContracts, usePoolContractBySymbol } from 'lib/hooks/usePoolContracts'
import { QUERY_KEYS } from 'lib/constants/queryKeys'
import { MAINNET_POLLING_INTERVAL } from 'lib/constants'
import { getPool, getPoolsByChainId } from 'lib/fetchers/getPools'
...
export const useAllPoolsKeyedByChainId = () => {
  ...
  const error = ethereumUseQuery.error || polygonUseQuery.error
  ...
  return { data, isFetched, isFetching, refetch, error }
}
...
const populatePerPoolCache = (chainId, queryClient, pools) =>
  pools.forEach((pool) => {
    pool.config.incentivizesSponsorship = _determineIncentivization(pool)
